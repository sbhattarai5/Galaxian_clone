...
int game_execute(Surface & surface, Event & event)
{
    ...
    int reset_i = 1;
    Sound sound("sounds/laser.wav");
    Sound exp_sound("sounds/explosion.wav");
    while (1)
    {
        int start = getTicks();
        KeyPressed keypressed = get_keypressed();
        ...
        if (keypressed[SPACE] && fleet->no_of_aliens_alive() != 0 && !spaceship_reset)
        {
            spaceship.shoot(laser_list);
            sound.play();
        }
        surface.lock();
        surface.fill(BLACK);
        ...
        check_collison(fleet, laser_list, spaceship, explosions, spaceship_reset, exp_sound);
...
bool start_screen(Surface & surface, Event & event)
{
    Color red = {255, 0 ,0};
    Font font("fonts/FreeSans.ttf", 30);
    char text1[] = "WE ARE GALAXIANS";
    char text2[] = "Hit Spacebar to play";
    Image img1(font.render(text1, red));
    Image img2(font.render(text2, WHITE));
    Rect rect1 = img1.getRect();
    rect1.x = W/2 - (rect1.w/2);
    Rect rect2 = img2.getRect();
    ...
    {
        KeyPressed keypressed = get_keypressed();    
        if (event.poll() && event.type() == QUIT) return false;
        surface.lock();
        surface.fill(BLACK);
        surface.put_image(img1, rect1);
        ++reset_time;
        if (reset_time <= reset_i * 300) surface.put_image(img2, rect2);
        else
        {
            if (reset_time % 300 == 0) reset_i += 2;
        }
        surface.unlock();
        surface.flip();
        if (keypressed[SPACE]) return true;
    }
}

bool end_screen(Surface & surface, int score, Event & event)
{
    Color red = {255, 0 ,0};
    Font font("fonts/FreeSans.ttf", 30);
    char text1[] = "Score: ";
    ...
    {
        text4[len - i - 1] = rev_score_text[i];
    }
    text4[len] ='\0';
    if (len == 0)
    {
    ...
    Image img4(font.render(text4, WHITE));
    Rect rect4 = img4.getRect();
    surface.put_image(img4, rect4);
    char text2[] = "Hit Spacebar to play again";
    char text3[] = "TAB to exit";
    Image img1(font.render(text1, red));
    Image img2(font.render(text2, WHITE));
    Image img3(font.render(text3, WHITE));
    Rect rect1 = img1.getRect();
    rect1.x = W/2 - (rect1.w/2);
    Rect rect2 = img2.getRect();
    rect2.x = W/2 - (rect2.w/2);
    rect1.y = H/2 - 20;
    rect2.y = H/2 + 20;
    Rect rect3 = img3.getRect();
    rect3.x = W/2 - (rect3.w/2);
    rect3.y = H/2 + 60;
    rect4.y = rect1.y;
    rect4.x = rect1.x + rect1.w + 20; 
    while (1)
    {
        KeyPressed keypressed = get_keypressed();
        if (event.poll() && event.type() == QUIT) return false;
        ...
        surface.fill(BLACK);
        surface.put_image(img1, rect1);
        surface.put_image(img2, rect2);
        surface.put_image(img3, rect3);
        surface.put_image(img4, rect4);
        surface.unlock();
        surface.flip();
        if (keypressed[SPACE]) return true;
        if (keypressed[TAB]) return false;
    }
...
void  game_loop()
{
    ...
    while (1)
    {
        if (game_state == -1)
        {
            user_choice = start_screen(surface, event);
